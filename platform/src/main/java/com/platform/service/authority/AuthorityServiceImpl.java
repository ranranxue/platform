...
public class AuthorityServiceImpl implements AuthorityService {
	...
	public BasicResponse saveManagerAuthority(saveManagerAuthorityRequest request) {
		...
		Integer updateNum=0;
		try {
			logger.debug("start to update manager manage grade");
			updateNum=manage_scopeDAO.updateManageGrade(request.getGrade(), request.getManagerId());
		} catch (Exception e) {
			// TODO: handle exception
		...
		if(updateNum==0){
			logger.debug("fail to update the manage grade!");
			BasicResponse response = new BasicResponse();
			response.setCode(ApiResultInfo.ResultCode.ServerError);
			response.setMsg(ApiResultInfo.ResultMsg.ServerError);
			return response;
		}else{
			logger.debug("update the manage grade successfullY!");
			BasicResponse response = new BasicResponse();
			response.setCode(0);
			response.setMsg("update the manage grade successfullY!");
			return response;
		}
	}
}
